...
EVP_CONFIG = {
    # --- 单图给分 ---
    "ABS_WEIGHT": 0.6198605471002996,       # 绝对部分权重 (rating)
    "REL_WEIGHT": 0.10,       # 相对部分权重 (rating/team_avg) —— 恢复: 保留队友相对惩罚
    "REL_CLIP_NEG": False,    # rel 负值是否归零: False = 保留负值惩罚 (全胜队内部也能区分谁相对平庸)
    "ABS_SCALE": 1.6598538459295487,         # F_abs = (rating - 1.0) * ABS_SCALE
    "REL_SCALE": 2.5719407526016784,         # F_rel = (rating/team_avg - 1.0) * REL_SCALE
    "GAMMA_ABS": 0.7599802622585494,         # 基础映射非线性度 (2026-08-29): F_abs = sign(x)·|x|^γ · ABS_SCALE, x=rating-1.
                              #   1.0=线性; <1 压缩高端(低端差异更陡, log 型); >1 放大高端(高端差异更陡).
    "GAMMA_REL": 0.7193722934071256,         # 相对部分同理, x = rating/team_avg - 1.
    "WIN_MULT": 1.7399540976680052,          # 胜利BO的胜利地图加成: 只有整场BO赢下时, 胜图才 × 此系数 (推动胜利进程)
    ...
    "LOSE_MULT": 1.0,       # 输图正分压缩: 输图 ms>0 时 × LOSE_MULT. 1.0=不压缩.
    "BO_LOSS_FACTOR": 1.0,    # 整场BO输掉时正分打折系数: 1.0 = 不打折 (负分照扣, 输 BO 不额外砍正分)
    "BO_LOSS_FACTOR_NORMAL": 1.0,  # 普通组(打了QF)输掉的BO正分打折: 败者组爬回的队伍, 输掉的BO含金量低.
                              #   1.0=与豁免组一致; 0.0=输掉的BO正分归零(治"输家还拿分", 只影响普通组, 豁免组保留)
    "MAP_SOFT_CAP": 1.784898741422197,      # 单图软上限: 超过后增速 -50% (平滑打折). 小组赛基准.
    "MAP_HARD_MAX": 2.059306121876244,      # 单图绝对上限: 达到后锁定为最大值. 小组赛基准.
    # --- 阶段化单图上限 (2026-08-29, 展开现有参数而非新机制) ---
    # 淘汰赛/决赛的单图上限 = 小组基准 × 对应乘子. 1.0 = 与小组一致 (round11 行为).
    "PO_SOFT_MULT": 0.8930796245623039,         # 淘汰赛 (Quarter/Semi, 不含 GF) 软上限乘子
    "PO_HARD_MULT": 1.2241002364631606,         # 淘汰赛硬上限乘子
    "GF_SOFT_MULT": 1.007234564595054,         # 决赛 (Grand final) 软上限乘子
    "GF_HARD_MULT": 1.12944220006418,         # 决赛硬上限乘子
    # --- BO 封顶 (最多/最少分) ---
    "CAP_BO1": 0.8553714828625412,           # BO1 最多 +1 分
    "CAP_BO3": 2.0286025997545845,           # BO3 最多 +2 分
    "CAP_BO5": 3.099048680143754,           # BO5 最多 +3 分
    ...
    "WIN_BO_FLOOR": -99.0,
    # --- 阶段差异化 ---
    "GROUP_MULT": 0.728596789412475,       # 小组赛封顶 = 淘汰赛 × 0.75 (BO3 小组赛 = 1.5)
    "GROUP_STAGES": ["Groups", "Online", "3rd place"],   # 3rd place 是安慰赛, 参照小组赛打折
    # --- 平滑软上限 (soft-landing + soft-saturate) ---
    "OVER_CAP_SLOPE": 0.5498955387798112,       # 淘汰赛 BO 超过 cap 后的增速比例 (0=硬切, 1=不压缩)
    "GROUP_OVER_CAP_SLOPE": 0.5464329169343723, # 小组赛 BO 超过 cap 后的增速比例: 豁免组(直接进四强, 全胜晋级, 神图是有效晋级图)不压
    "GROUP_OVER_CAP_SLOPE_NORMAL": 0.24686885878448475,  # 小组赛 BO 超额收紧: 普通组(打了 QF, 败者组爬回)的神图累积压更狠
    "SOFT_LAND_SPREAD": 0.16061175128783736, # cap 后软着陆宽度 = cap 的倍数 (过渡区)
    "SATURATE_TAU": 0.3184801903103723,      # 普通组小组 BO 超额指数衰减时间常数 (None=不启用 saturate, 走斜率)
    "SATURATE_R0": 0.33988330002827927,      # saturate 衰减段的起始增速
    "SATURATE_FLAT": 0.29501610600013245,     # 超额豁免区: 超额 ≤ flat 完全按 soft_cap 给足 (小额神图照给), 超过才衰减.
    ...
    "GROUP_CAP_EXEMPT": {"soft": 4.154666624117872, "xmax": 7.017645734747101, "slope": 0.2485505631587899},
    # 普通组(打了QF)总 cap: xmax 提到 8.0/slope 0.4 后, 8248 的 donk(组原始高)能从 5.0 附近涨到 5.2,
    #   且 8240 无损 (YEKINDAR 组 1.87 远低于 soft=4.0, 完全不受影响)
    "GROUP_CAP_NORMAL": {"soft": 4.054652389921319, "xmax": 7.83165190673868, "slope": 0.6377664367298718},
    # --- 豁免组补偿 (简化替代双轨 cap 差异) ---
    ...
    "EXEMPT_COMP": 1.2337644458678962,         # 1.0=不补偿; >1 = 豁免组小组分在 cap 前 × 该系数
    ...
    "PLAYOFF_CAP_EXEMPT": {"soft": 6.5752, "xmax": 9.8628, "slope": 0.1},
    "PLAYOFF_CAP_NORMAL": {"soft": 6.4835, "xmax": 9.72525, "slope": 0.1},
    # 纯淘汰赛赛事补偿 (2026-08-30): 无小组赛的赛事 (BLAST赏金赛 8246 / EPL S23 8413)
    # 总分 = 淘汰赛单段, 缺"小组段"垫底 → 系统性比"小组+淘汰两段"的正常赛事低 ~0.8×.
    # 淘汰分乘 NO_GROUP_MULT 补偿缺失的小组段. TPE 纯 obj 收敛 1.533, 但把 makazze(8413)
    # 原始淘汰分 7.33 放大到 11.2 (手动 8.9). 用户 2026-08-30 定为 1.3: makazze 9.53 缓解虚高,
    # 高分档均值 +0.348→+0.258, obj 107.41→107.03. 注意: 8246 nota(原始 3.26)在 1.3 下=4.240,
    # 仅领先 m0NESY 0.001 (噪声级), 5 分线已不合格 — 纯淘汰赛乘子两难 (乘子高→makazze 虚高,
    # 乘子低→nota 掉线), 1.3 为折中.
    "NO_GROUP_MULT": 1.3,
    ...
    "LEN_PENALTY": 0.006742854523866354,     # 每超出 1 张胜利图, 累积总价值缩减比例 (0.8%): 只轻罚"多打才晋级"的弯路
    ...
    "ESL_GSL_FACTOR": 0.6863416482231273,  # ESL/GSL 下多余胜利图的折算系数 (调优确定)
}
