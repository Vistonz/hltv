...
EVP_CONFIG = {
    # --- 单图给分 ---
    "ABS_WEIGHT": 1.193849193268,       # 绝对部分权重 (rating)
    "REL_WEIGHT": 0.090715504642,       # 相对部分权重 (rating/team_avg) —— 恢复: 保留队友相对惩罚
    "REL_CLIP_NEG": False,    # rel 负值是否归零: False = 保留负值惩罚 (全胜队内部也能区分谁相对平庸)
    "ABS_SCALE": 0.692555865034,         # F_abs = (rating - 1.0) * ABS_SCALE
    "REL_SCALE": 2.003597879545,         # F_rel = (rating/team_avg - 1.0) * REL_SCALE
    "GAMMA_ABS": 0.831549551792,         # 基础映射非线性度 (2026-08-29): F_abs = sign(x)·|x|^γ · ABS_SCALE, x=rating-1.
                              #   1.0=线性; <1 压缩高端(低端差异更陡, log 型); >1 放大高端(高端差异更陡).
    "GAMMA_REL": 1.522594440952,         # 相对部分同理, x = rating/team_avg - 1.
    "WIN_MULT": 2.356894632424,          # 胜利BO的胜利地图加成: 只有整场BO赢下时, 胜图才 × 此系数 (推动胜利进程)
    ...
    "LOSE_MULT": 1.422226137415,       # 输图正分压缩: 输图 ms>0 时 × LOSE_MULT. 1.0=不压缩.
    "BO_LOSS_FACTOR": 1.137439652708,    # 整场BO输掉时正分打折系数: 1.0 = 不打折 (负分照扣, 输 BO 不额外砍正分)
    "BO_LOSS_FACTOR_NORMAL": 0.993087886791,  # 普通组(打了QF)输掉的BO正分打折: 败者组爬回的队伍, 输掉的BO含金量低.
                              #   1.0=与豁免组一致; 0.0=输掉的BO正分归零(治"输家还拿分", 只影响普通组, 豁免组保留)
    "MAP_SOFT_CAP": 0.911950946659,      # 单图软上限: 超过后增速 -50% (平滑打折). 小组赛基准.
    "MAP_HARD_MAX": 1.981806130027,      # 单图绝对上限: 达到后锁定为最大值. 小组赛基准.
    # --- 阶段化单图上限 (2026-08-29, 展开现有参数而非新机制) ---
    # 淘汰赛/决赛的单图上限 = 小组基准 × 对应乘子. 1.0 = 与小组一致 (round11 行为).
    "PO_SOFT_MULT": 0.491116952482,         # 淘汰赛 (Quarter/Semi, 不含 GF) 软上限乘子
    "PO_HARD_MULT": 1.301581406713,         # 淘汰赛硬上限乘子
    "GF_SOFT_MULT": 1.400821375094,         # 决赛 (Grand final) 软上限乘子
    "GF_HARD_MULT": 1.000205039762,         # 决赛硬上限乘子
    # --- BO 封顶 (最多/最少分) ---
    "CAP_BO1": 1.21815660924,            # BO1 单场封顶 (2026-08-31 新口径调优 ×2.5)
    ...
    "CAP_BO3": 1.998072416682,           # BO3 最多 +2 分
    "CAP_BO5": 3.252425084552,           # BO5 最多 +3 分
    ...
    "WIN_BO_FLOOR": -99.0,
    # --- 阶段差异化 ---
    "GROUP_MULT": 1.758996319612,       # 小组赛封顶 = 淘汰赛 × 0.75 (BO3 小组赛 = 1.5)
    "GROUP_STAGES": ["Groups", "Online", "3rd place"],   # 3rd place 是安慰赛, 参照小组赛打折
    # --- 平滑软上限 (soft-landing + soft-saturate) ---
    "OVER_CAP_SLOPE": 0.676827602744,       # 淘汰赛 BO 超过 cap 后的增速比例 (0=硬切, 1=不压缩)
    "GROUP_OVER_CAP_SLOPE": 0.936503480749, # 小组赛 BO 超过 cap 后的增速比例: 豁免组(直接进四强, 全胜晋级, 神图是有效晋级图)不压
    "GROUP_OVER_CAP_SLOPE_NORMAL": 0.079971246891,  # 小组赛 BO 超额收紧: 普通组(打了 QF, 败者组爬回)的神图累积压更狠
    "SOFT_LAND_SPREAD": 0.305180420696, # cap 后软着陆宽度 = cap 的倍数 (过渡区)
    "SATURATE_TAU": 0.47905795247,      # 普通组小组 BO 超额指数衰减时间常数 (None=不启用 saturate, 走斜率)
    "SATURATE_R0": 0.352040806263,      # saturate 衰减段的起始增速
    "SATURATE_FLAT": 0.343414542282,     # 超额豁免区: 超额 ≤ flat 完全按 soft_cap 给足 (小额神图照给), 超过才衰减.
    ...
    "GROUP_CAP_EXEMPT": {"soft": 2.338997205114, "xmax": 10.84661318283, "slope": 0.266352916192},
    # 普通组(打了QF)总 cap: xmax 提到 8.0/slope 0.4 后, 8248 的 donk(组原始高)能从 5.0 附近涨到 5.2,
    #   且 8240 无损 (YEKINDAR 组 1.87 远低于 soft=4.0, 完全不受影响)
    "GROUP_CAP_NORMAL": {"soft": 2.283924491621, "xmax": 11.854852031445, "slope": 0.436352538175},
    # --- 豁免组补偿 (简化替代双轨 cap 差异) ---
    ...
    "EXEMPT_COMP": 1.781043182575,         # 1.0=不补偿; >1 = 豁免组小组分在 cap 前 × 该系数
    ...
    "PLAYOFF_CAP_EXEMPT": {"soft": 5.671453926509, "xmax": 16.856085210659, "slope": 0.1},
    "PLAYOFF_CAP_NORMAL": {"soft": 6.284400685595, "xmax": 5.507861234155, "slope": 0.1},
    ...
    "LEN_PENALTY": 0.023805066561,     # 每超出 1 张胜利图, 累积总价值缩减比例 (1.2%):
    ...
    "ESL_GSL_FACTOR": 1.094564130458,  # ESL/GSL 下多余胜利图的折算系数 (调优确定)
}

# ----------------------------------------------------------------------
# 0b. CS2 段独立配置 (两套分拆, 2026-09-09 用户指令 "拆成两套")
# ----------------------------------------------------------------------
# EVP_CONFIG = 两段共用的 42 核心参数 (永不含 CS2 机制键).
# CS2 段 = EVP_CONFIG + CS2_OVERRIDES (cfg_for merge). CS2_OVERRIDES 可覆写任意键:
#   机制轴权重 (CS2_<AXIS>_W) 在此; 若未来要 CS2 独立漂移, 42 核心参数也可放这里 —
#   CS:GO 段仍不 merge (段间隔离在架构层面而非仅结构性).
#
# ⚠ 历史 (2026-09-10): 此处原为 "CS:GO 逐字节冻结源"; 用户 2026-09-10 决策「C 解冻写回」
#   后该冻结策略**已解除** —— 42 核参数重新是 CS:GO/CS2 共用可调参数 (本轮精调 obj 115.4474
#   即写回在此, 190 个 CS:GO 交付物随之重生成)。详见记忆 evp-final-config-20260910。
#
...
#     专属列 (CS:GO raw 结构无这些列 → 机制在 CS:GO 天然永不触发; 这是数据结构性事实,
#     与 2026-09-10 已解除的 CS:GO 冻结策略无关).
CS2_OVERRIDES = {
...
def cfg_for(segment, base=None, cs2_overrides=None):
    """产品段配置: segment=='cs2' → base + CS2 覆写; 否则 → base (原样, 不 merge).

    cs2_overrides=None → 模块 CS2_OVERRIDES (生产默认);
    {} → 强制机制关 (闸门基线); dict → 自定义覆写 (搜索逐点传入).
    base=None → EVP_CONFIG. 返回 dict 副本, 不污染调用方.
    """
