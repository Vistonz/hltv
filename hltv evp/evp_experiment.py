...
EVP_CONFIG = {
    # --- 单图给分 ---
    "ABS_WEIGHT": 0.8766202067847035,       # 绝对部分权重 (rating)
    "REL_WEIGHT": 0.09408735633724744,       # 相对部分权重 (rating/team_avg) —— 恢复: 保留队友相对惩罚
    "REL_CLIP_NEG": False,    # rel 负值是否归零: False = 保留负值惩罚 (全胜队内部也能区分谁相对平庸)
    "ABS_SCALE": 0.882631892223841,         # F_abs = (rating - 1.0) * ABS_SCALE
    "REL_SCALE": 1.8381088369642604,         # F_rel = (rating/team_avg - 1.0) * REL_SCALE
    "GAMMA_ABS": 0.7940488590777547,         # 基础映射非线性度 (2026-08-29): F_abs = sign(x)·|x|^γ · ABS_SCALE, x=rating-1.
                              #   1.0=线性; <1 压缩高端(低端差异更陡, log 型); >1 放大高端(高端差异更陡).
    "GAMMA_REL": 1.2412881629803185,         # 相对部分同理, x = rating/team_avg - 1.
    "WIN_MULT": 2.598694466498432,          # 胜利BO的胜利地图加成: 只有整场BO赢下时, 胜图才 × 此系数 (推动胜利进程)
    ...
    "LOSE_MULT": 1.4044215952192638,       # 输图正分压缩: 输图 ms>0 时 × LOSE_MULT. 1.0=不压缩.
    "BO_LOSS_FACTOR": 1.3068810112721891,    # 整场BO输掉时正分打折系数: 1.0 = 不打折 (负分照扣, 输 BO 不额外砍正分)
    "BO_LOSS_FACTOR_NORMAL": 1.1378531748100427,  # 普通组(打了QF)输掉的BO正分打折: 败者组爬回的队伍, 输掉的BO含金量低.
                              #   1.0=与豁免组一致; 0.0=输掉的BO正分归零(治"输家还拿分", 只影响普通组, 豁免组保留)
    "MAP_SOFT_CAP": 0.8679708183403765,      # 单图软上限: 超过后增速 -50% (平滑打折). 小组赛基准.
    "MAP_HARD_MAX": 1.8337815088870861,      # 单图绝对上限: 达到后锁定为最大值. 小组赛基准.
    # --- 阶段化单图上限 (2026-08-29, 展开现有参数而非新机制) ---
    # 淘汰赛/决赛的单图上限 = 小组基准 × 对应乘子. 1.0 = 与小组一致 (round11 行为).
    "PO_SOFT_MULT": 0.33406820093654016,         # 淘汰赛 (Quarter/Semi, 不含 GF) 软上限乘子
    "PO_HARD_MULT": 1.30403223870157,         # 淘汰赛硬上限乘子
    "GF_SOFT_MULT": 0.9997133420588827,         # 决赛 (Grand final) 软上限乘子
    "GF_HARD_MULT": 1.0187235617531551,         # 决赛硬上限乘子
    # --- BO 封顶 (最多/最少分) ---
    "CAP_BO1": 0.8045988508037966,            # BO1 单场封顶 (2026-08-31 新口径调优 ×2.5)
    ...
    "CAP_BO3": 1.819286096200348,           # BO3 最多 +2 分
    "CAP_BO5": 3.202002593773055,           # BO5 最多 +3 分
    ...
    "WIN_BO_FLOOR": -99.0,
    # --- 阶段差异化 ---
    "GROUP_MULT": 1.6827160074671408,       # 小组赛封顶 = 淘汰赛 × 0.75 (BO3 小组赛 = 1.5)
    "GROUP_STAGES": ["Groups", "Online", "3rd place"],   # 3rd place 是安慰赛, 参照小组赛打折
    # --- 平滑软上限 (soft-landing + soft-saturate) ---
    "OVER_CAP_SLOPE": 0.589000678961527,       # 淘汰赛 BO 超过 cap 后的增速比例 (0=硬切, 1=不压缩)
    "GROUP_OVER_CAP_SLOPE": 0.5129165480467109, # 小组赛 BO 超过 cap 后的增速比例: 豁免组(直接进四强, 全胜晋级, 神图是有效晋级图)不压
    "GROUP_OVER_CAP_SLOPE_NORMAL": 0.14382193220805553,  # 小组赛 BO 超额收紧: 普通组(打了 QF, 败者组爬回)的神图累积压更狠
    "SOFT_LAND_SPREAD": 0.3046981652844689, # cap 后软着陆宽度 = cap 的倍数 (过渡区)
    "SATURATE_TAU": 0.7355455212514294,      # 普通组小组 BO 超额指数衰减时间常数 (None=不启用 saturate, 走斜率)
    "SATURATE_R0": 0.5580122205012237,      # saturate 衰减段的起始增速
    "SATURATE_FLAT": 0.45919509612739695,     # 超额豁免区: 超额 ≤ flat 完全按 soft_cap 给足 (小额神图照给), 超过才衰减.
    ...
    "GROUP_CAP_EXEMPT": {"soft": 2.1741927042775333, "xmax": 10.198987391601975, "slope": 0.28381923207251236},
    # 普通组(打了QF)总 cap: xmax 提到 8.0/slope 0.4 后, 8248 的 donk(组原始高)能从 5.0 附近涨到 5.2,
    #   且 8240 无损 (YEKINDAR 组 1.87 远低于 soft=4.0, 完全不受影响)
    "GROUP_CAP_NORMAL": {"soft": 2.8095429029672703, "xmax": 17.40235983972551, "slope": 0.45542947951017954},
    # --- 豁免组补偿 (简化替代双轨 cap 差异) ---
    ...
    "EXEMPT_COMP": 1.9232467538533118,         # 1.0=不补偿; >1 = 豁免组小组分在 cap 前 × 该系数
    ...
    "PLAYOFF_CAP_EXEMPT": {"soft": 9.695093163112231, "xmax": 12.722947583238387, "slope": 0.1},
    "PLAYOFF_CAP_NORMAL": {"soft": 6.86789332273084, "xmax": 3.6782309851313886, "slope": 0.1},
    ...
    "NO_GROUP_MULT": 1.3953608087639884,
    ...
    "LEN_PENALTY": 0.034040270629526234,     # 每超出 1 张胜利图, 累积总价值缩减比例 (1.2%):
    ...
    "ESL_GSL_FACTOR": 1.1637262372892647,  # ESL/GSL 下多余胜利图的折算系数 (调优确定)
}
