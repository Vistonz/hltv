...
file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rating2026.xlsx") #表格文件保存路径，根据需要更改
...
def run_yearly_scrape(
    webfront=webfront,
    eventfilter=eventfilter,
    bigeventfilter=bigeventfilter,
    eliteeventfilter=eliteeventfilter,
    supereliteeventfilter=supereliteeventfilter,
    ...
    """年度评分爬虫主入口: 抓所有赛事/单场/残局/武器数据, 汇总写 rating2026.xlsx.

    参数均可传入覆盖 (默认值 = 脚本内硬编码配置, 行为不变):
      webfront / 各 eventfilter / arenafilter  统计页 URL 与赛事筛选
    ...
    # 模块化 (嵌套辅助函数, 闭包引用 driver/name/row/keyword)
