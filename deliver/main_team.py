...
职责：
  ...
  2. 找到 exe 旁边的“配置_team.ini”，没有则用内置模板生成一份；
  3. 把配置翻译成 hltvteam.scrape_team_stats 需要的参数并调用；
  4. 任何错误都翻译成面向零基础用户的中文提示，最后暂停等待回车。

注意：抓取/计算逻辑全部在 hltvteam.py 里，本文件不改动它任何口径。
"""
...
import hltvteam as core            # noqa: E402  核心爬虫（原封复制，未改动）
import shared_lib as sl           # noqa: E402  共享工具

# ---- 与源脚本“初始数据区”锁定的常量（保持不变，勿改）-----------------
WEBFRONT = "https://www.hltv.org/stats/teams"     # 队伍统计页 URL 前缀
KEYWORD = ">"                                     # 清洗 name/rating 的分隔符
CONFIG_NAME = "配置_team.ini"
OUT_DEFAULT_NAME = "teamstat.xlsx"
...
def build_kwargs(parser):
    ...
    键名与 hltvteam.scrape_team_stats 的形参一一对应。
    """
    base = sl.app_dir()
...
def main():
    ...
    try:
        ...
        print("每个队伍页需要等待约 20 秒加载，请耐心等待……\n")

        core.scrape_team_stats(**kwargs)
